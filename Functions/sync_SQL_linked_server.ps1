function sync_SQL_linked_server {
    ...
    $filename = "MetaDataSync__" + $($Script.Replace('.ps1', '')) + "__" + $primary_server.Replace('/', '_') + "__ScriptOut__$Dtmm.sql"
    ...
    elseif ($primary_server.Toupper().contains(".DESHAW.COM")) { $primary = $primary_server.substring(0, $primary_server.IndexOf(".")) }
    ...
    elseif ($dr_server.Toupper().contains(".DESHAW.COM")) { $dr = $dr_server.substring(0, $dr_server.IndexOf(".")) }
...
select * into sandbox..sys_servers_$dr from sys.servers;"
    ...
    if ($missing_server_list.Count -gt 0) {
        ...
        $Body = $body + "-- Alert generated from MetaDataSync Module.`n"
        $message = $Body

        if(-not $WhatIf) {
            Raise-DbaServiceNowAlert -Summary "MetaDataSync-LinkedServerMissing-$primary_server" -Severity HIGH -Description $message `
...
"@
    ...
    if ($discrepancy_config_servers.Count -gt 0) {
        ...
        if(-not $WhatIf) {
            Raise-DbaServiceNowAlert -Summary "MetaDataSync-LinkedServerConfig-$primary_server" -Severity HIGH -Description $message `
                                -AlertSourceHost $primary_server -AlertTargetHost $primary_server `
                                -Alertkb $alertKB
        }
        else {
