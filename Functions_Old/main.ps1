...
AND    ServerType = 'DB'
AND    HasDR = 'Yes'
AND    Dataserver not like '%SQLPROD%' and Dataserver not like '%DBREPLTOA1.deshaw.com,13601%' and Dataserver not like 'SHAREDMGMT3.win.lab.com%'
    ...
    foreach ($server in $server_list) {
        ...
        Write-Host "`n`n`nStarting meta_data_sync for PROD: $prod_server, DR: $dr_server`n" -ForegroundColor Cyan
