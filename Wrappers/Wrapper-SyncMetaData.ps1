...
if([string]::IsNullOrEmpty($PSScriptRoot)) {
    $PSScriptRoot = 'C:\Users\Public\Documents\git\dba\DOMAIN1\powershell\modules\MetaDataSync\Wrappers'
}
Import-Module $PsScriptRoot\..\..\DbaHelpUtil -DisableNameChecking
Import-Module $PsScriptRoot\..\MetaDataSync -DisableNameChecking
$Error.Clear();
...
if ([String]::IsNullOrEmpty($OutputPath)) {
    ...
    $OutputPath = "$PsScriptRoot\..\..\..\Logs\MetaDataSync"
...
if (Test-Path -Path $OutputPath) {
    ...
    Get-ChildItem -Path $(Join-Path $OutputPath "MetaDataSync*.sql") `
        | Where-Object {$_.PSIsContainer -eq $false -and $_.LastWriteTime -lt (Get-Date).AddDays(-$LogRetentionDays) } `
        | Remove-Item -WhatIf:$WhatIf -Verbose | Tee-Object $logFileFullPath -Append | Write-Output
}
...
SELECT p.Dataserver AS prod_data_server, p.FriendlyName as prod_friendly_name
	...
	,1 AS is_meta_data_sync_enabled
...
UNION ALL
--
SELECT 'SQLPROD1DS6.deshaw.com,14346' as prod_data_server, 'SQLPROD1DS6' as prod_friendly_name,
		'SQLDR1DS6.deshaw.com,14356' as dr_data_server, 'SQLDR1DS6' as dr_friendly_name
		,1 AS is_meta_data_sync_enabled
"@
    }
    #>
    ...
    $server_list_filtered += $server_list #| Where-Object {$_.prod_friendly_name -in @('SQLPROD1DS6')}
    foreach ($server in $server_list_filtered)
    {
        ...
        try
        {
            ...
            #if($prod_name -in @('SQLPROD1DS2','SQLPROD1DS6')) { 1/0 }
...
catch
{
    ...
    $Body = "Alert: MetaDataSync failed for $Environment`n";
    $Body = $Body + "Execution Time: $([Math]::Ceiling($durationSpan.TotalMinutes)) minutes `n";
    $Body = $Body + "View Alert Status: '(dba) Invoke-MetaDataSync' job on DBMONITOR `n";
    $Body = $Body + "Alert Parameters:`n";
    $Body = $Body + "Description: MetaDataSync failed for few servers in $Environment environment.`nPlease find logs in `"$logFileFullPath`".";
    $Body = $Body + "`nBelow are error details-`n`n$errMessage`n`n";
    $Body = $Body + "`n`nNote: A reminder email will be sent.`n";
    $Subject = "MetaDataSync-$Environment-Failed";
